...
**DO NOT use built-in `WebSearch` - it does NOT work with GLM coding plan.**
...
## Sources
- [Title 1](URL1) - [Brief description]
- [Title 2](URL2) - [Brief description]
- [Title 3](URL3) - [Brief description]
```
...
**Parallel Fallback:** If primary fails, try remaining tools in parallel. Return whatever succeeds.
...
Firecrawl → Diverse perspectives, when Exa rate limited
```
...
| Don't | Do Instead |
|-------|------------|
| Use Ref for news/current events | Use Exa or Firecrawl |
| Use Firecrawl when you need full content now | Use Exa (saves scraping) |
| Use only one tool for comprehensive research | Run multiple tools parallel |
| Ignore rate limits and retry same tool | Fall back to backup |
| Use built-in WebSearch | Use MCP search tools |
| Use Exa when you just need official docs URL | Use Brave or Ref (faster) |
...
├─────────────────────────────────────────────────────────────┤
│  NEWS/BREAKING?        →  Exa → Firecrawl                   │
│  FULL CONTENT NOW?     →  Exa (15K tokens)                  │
│  OFFICIAL DOCS?        →  Brave OR Ref                      │
│  GITHUB REPO?          →  Ref → Brave site:github           │
│  CODE EXAMPLES?        →  Exa get_code_context_exa          │
│  LEARN CONCEPT?        →  Exa → Brave                       │
│  DEBUG ERROR?          →  Brave → Exa                       │
│  COMPREHENSIVE?        →  ALL 4 in parallel                 │
│  UNSURE?               →  Exa (safest default)              │
│                                                             │
│  RATE LIMITED?         →  Use next tool in backup chain     │
├─────────────────────────────────────────────────────────────┤
│  80% of searches: Exa    20% of searches: Brave/Ref         │
└─────────────────────────────────────────────────────────────┘
```

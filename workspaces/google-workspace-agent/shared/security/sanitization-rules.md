...
| Priority manipulation | `<!--\s*PRIORITY[_\s]*(HIGH|CRITICAL|URGENT).*?-->` | REMOVE |
| Hidden category injection | `<!--\s*CATEGORY[:\s].*?-->` | REMOVE |
| Metadata injection | `<!--\s*(urgent|important|flagged).*?-->` | REMOVE |
...
| Pattern | Regex | Action |
|---------|-------|--------|
| Fake urgency markers | `URGENT:{10,}` | NORMALIZE |
| False reply indicators | `Re:{5,}` | NORMALIZE |
| Spoofed headers | `From:.*?(attacker|malicious)` | FLAG |
| Hidden forwarding | `<!--\s*forward.*?-->` | REMOVE |
...
| Quote manipulation | `>{10,}` | TRUNCATE to `>>>` |
